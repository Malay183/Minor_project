...
class Minor_proj(tk.Frame): 
    ...
    def calculate(self):
        """calculating the time entry provided by the user in numeric form to time format"""
    ...
    def timer(self):
        """display the time calculated after user input"""
        if self.running:
            if self.time <= 0:
    ...
    def quit(self):
        """nwo if the user wants to quit the timer"""
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("TIMER")
    Minor_proj(root).pack(side="top", fill="both", expand=True)
    root.mainloop()
